...
<c:if test="${rbac:hasPermission(activeUserInfo.id, '/organization/editPage')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${rbac:hasPermission(activeUserInfo.id, '/organization/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<script type="text/javascript">
	...
	$(function() {
		treeGrid = $('#treeGrid').treegrid({
			url : '${baseUrl}/organization/organizationList',
			idField : 'id',
			treeField : 'name',
			parentField : 'pid',
			fit : true,
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 500,
			height : 350,
			href : '${baseUrl}/organization/addPage',
			buttons : [ {
				width : 72,
				text : '添加',
				handler : function() {
		...
		if (node) {
			parent.$.messager.confirm('询问', '您是否要删除当前机构？', function(b) {
				if (b) {
					...
					$.post('${baseUrl}/organization/delete', {
						id : node.id
					}, function(result) {
						if (result.success) {
	...
	function editFun(id) {
		...
		if (node) {
			parent.$.modalDialog({
				...
				width : 500,
				height : 300,
				href : '${baseUrl}/organization/editPage?id=' + node.id,
				buttons : [ {
					text : '编辑',
					handler : function() {
						parent.$.modalDialog.openner_treeGrid = treeGrid;//因为添加成功之后，需要刷新这个treeGrid，所以先预定义好
	...
	function grantFun(id) {
		...
		if (node) {
			parent.$.modalDialog({
				...
				width : 500,
				height : 500,
				href : '${baseUrl}/organization/grantPage?id=' + node.id,
				buttons : [ {
					text : '授权',
					handler : function() {
						parent.$.modalDialog.openner_treeGrid = treeGrid;//因为添加成功之后，需要刷新这个treeGrid，所以先预定义好
...
<body>
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${rbac:hasPermission(activeUserInfo.id, '/organization/addPage')}">
