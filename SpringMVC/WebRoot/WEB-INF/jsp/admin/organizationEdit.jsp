...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:false" title="" style="overflow: hidden;">
		<form id="form" method="post">
			<input name="id" value="${organization.id}" style="display: none;">
			<table class="table table-hover table-condensed">
				<tr>
					<%-- <th>编号</th>
					<td><input name="id" type="text" class="span2" value="${organization.id}" readonly="readonly"></td> --%>
					<th>机构名称</th>
					<td><input name="name" type="text" placeholder="请输入机构名称" class="easyui-validatebox span2" data-options="required:true" value="${organization.name}"></td>
					<th>机构地址</th>
					<td><input name="address" type="text" placeholder="请输入机构地址" class="easyui-validatebox span2" value=""></td>
				</tr>
				<tr>
					<th>排序</th>
					<td><input name="seq" value="${organization.seq}" class="easyui-numberspinner" style="width: 140px; height: 29px;" required="required" data-options="editable:false"></td>
					<th>上级机构</th>
				...
				<tr>
					...
					<td><select name="status" class="easyui-combobox" data-options="width:140,height:29,editable:false,panelHeight:'auto'">
							<c:forEach items="${statusTypes}" var="statusType">
          						<option value="${statusType.value}" <c:if test="${organization.status==statusType.value}">selected</c:if>>${statusType.text}</option>
							</c:forEach>
						</select></td>
				</tr>
				<tr>
					<th>机构描述</th>
					<td colspan="3"><textarea name="description" rows="" cols="" class="span5">${organization.description}</textarea></td>
				</tr>
			</table>
		</form>
	</div>
